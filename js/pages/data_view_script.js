(function() {
  ...
  data_grid.view = function(id, data, options) {
    var filter_id_num, grid, grid_body, grid_filters, grid_header, grid_row, grid_rows, grid_titles, header, header_data, _i, _j, _k, _len, _len2, _len3, _results;
    ...
    grid_rows = grid_body.selectAll("tr").data(data);
    grid_row = grid_rows.enter().append("tr");
    _results = [];
    for (_k = 0, _len3 = header_data.length; _k < _len3; _k++) {
      header = header_data[_k];
  ...
  data_grid.show = function(data, options) {
    ...
    return data_grid.view("#data_grid_data", page_data, options);
  };
  $(function() {
    var error_no_file, options, show_grid;
    options = data_grid.get_options();
    show_grid = function(data) {
    ...
    if (options.filename) {
      return d3.csv(options.filename, show_grid);
    } else {
